function sortChange(currencyChange, amount) {
  let filter = [];
  let counter = 0;
...
function findChange(monedasDisponiblesParaElCambio, change, amount) {
  let currencyChange = [];
...
function findCurrencyLowerOrEqual(currC, cid) {
...
function checkCashRegister(price, cash, cid) {
  ...
  let findCResult = findCurrencyLowerOrEqual(currC, cid);
  let monedasDisponiblesParaElCambio = findCResult[0];
  let amount = findCResult[1];
  let cajaIni = findCResult[2];
  ...
  // si es mayor o igual:
  let changeResult = findChange(monedasDisponiblesParaElCambio, change, amount);
  let currencyChange = changeResult[0];
  let restanteEnCaja = changeResult[1];
  change = changeResult[2];
